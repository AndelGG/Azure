from src.anime.schemas import AnimeSchema, AnimeBannerSchema, ShortAnimeSchema, EpisodeSchema, AnimeTranslationSchema
from src.KodikAPI import MaterialData
from src.tags.schemas import TagsScheme
from src.unitofwork import IUnitOfWork


async def _get_translations_by_id(uow: IUnitOfWork, id: int) -> list[AnimeTranslationSchema]:
...
class AnimeService:
    async def get_banner_anime(self, uow: IUnitOfWork, count: int) -> list[AnimeBannerSchema]:
        search_response = await uow.movie_api.get_popular(min(count * 3, 100), {"has_field":"shikimori_id"})
    ...
    async def get_translation(self, uow, shiki_id, translation_id):
        translation = await uow.movie_api.get_series_by_id(shiki_id, translation_id)
        if not translation:
            return None
    ...
    async def get_popular_anime(self, uow: IUnitOfWork, count: int) -> list[ShortAnimeSchema]:
        search_response = await uow.movie_api.get_popular(min(count * 3, 100), {"has_field":"shikimori_id"})
        search_results = list()
        search_slug = set()
    ...
    async def get_anime_by_slug(self, uow, slug: str) -> AnimeSchema:
        ...
        if material.shikimori_id:
            translations = await _get_translations_by_id(uow, material.shikimori_id)
        else:
            translations = None
    ...
    async def get_short_by_search(self, uow: IUnitOfWork, search: str, limit: int) -> list[ShortAnimeSchema]:
